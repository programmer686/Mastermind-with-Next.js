...
export default function GameBoard() {
    ...
    const [toManyCharacters, setToManyCharacters] = useState("");
    function handleInput(e) {
        const {value} = e.target
        if (value.length > 4) {
            setToManyCharacters("You can only enter four characters at a time")
            break;
        }
    }
    ...
    return (
        <main>
            ...
            <div className={styles.GameBoard__body}>
                <PastPlay/>
                <GamePlay handleInput={handleInput} toManyCharacters={true}/>
            </div>
        </main>
    )
}